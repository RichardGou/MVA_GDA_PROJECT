...
def plot_loss_landscape(grid_x, grid_y, loss_values, pca_result, trained_loss, trained_test_loss_sqrt, ms, params_sqrt, pca_basis, center_point):
    ...
    #pca_proj_sqrt_y = np.dot(params_sqrt, pca_basis[1].T)
    ...
    fig.write_html(f"/home/bibahaduri/MVA_GDA_visuals/MVA_GDA_visuals_comb{ms}")
    #fig.show()
...
for k in range(ntrials):
    ...
    for i, m in enumerate(ms):
        ...
        plot_loss_landscape(grid_x, grid_y, loss_values, pca_result, trained_test_losses, trained_test_loss_sqrt,m,final_params_all_sqrt, pca_basis, stacked_params)
